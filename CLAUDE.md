...
- `.github/workflows/St1_Build-x86_64-release.yml`：下载 KoolCenter 的 x86_64 ImageBuilder，使用 `x86_64/build24.sh` 构建 `x86/64` 的通用 `.img.gz`/manifest 产物并上传到独立 Release；x86_64 不经过 ophub 打包，默认集成 kenzok8/openwrt-daede 的 `daed`、`luci-app-daede` 和匹配的 `vmlinux-btf`。
...
- `x86_64/build24.sh`：x86_64 构建入口。默认复用 x86_64 ImageBuilder profile 自带官方包集合，只追加 `x86_64/package-lists/` 和 `CUSTOM_PACKAGES`，并在请求 `dae`/`daed`/`luci-app-daede`/`vmlinux-btf` 时下载对应预编译 ipk 到 ImageBuilder 本地源，避免混用 arm64 包清单或 aarch64 本地 `.ipk`。
...
- 版本号集中在 `.github/build-config.env` 的 `VERSION`；arm64 还要同步检查 `arm64/repositories.conf` 的 OpenWrt 版本/内核 kmods URL。升级 iStoreOS/OpenWrt 版本时要确认 St1 Release tag 与 St2 下载 URL 仍匹配，并检查 `X86_64_IMAGEBUILDER_URL`、`X86_64_VMLINUX_BTF_VERSION`、`X86_64_VMLINUX_BTF_SHA256` 是否匹配期望的 x86_64 ImageBuilder 和内核。
